...
;(async () => {
  ...
  for (const item of example) {
    ...
    if (item.includes('midway-vue3-ssr')) {
      ...
      ln(
        '-s',
        resolve(path, './node_modules/vue'),
        resolve(cwd, './node_modules/vue-template-compiler/node_modules/vue')
      )
      ln(
        '-s',
        resolve(path, './node_modules/vue'),
        resolve(cwd, './node_modules/vue-server-renderer/node_modules/vue')
      )
    }
