import { h, createSSRApp, renderSlot, type VNode } from 'vue'
import {
  findRoute,
  getManifest,
  logGreen,
  ...
  const getApp = ({
    ...
    const app = createSSRApp({
      render: function () {
        ...
        const commonInject = `window.__USE_VITE__=${isVite}; window.prefix="${prefix}";${
          clientPrefix ? `window.clientPrefix="${clientPrefix}"` : ''
        };${isDev ? `window.ssrDevInfo=${JSON.stringify(ssrDevInfo)}` : ''}`
        const initialData = !isCsr
          ? h('script', {
              innerHTML: `window.__USE_SSR__=true; window.__INITIAL_DATA__ = ${serialize(
                state
              )};window.__INITIAL_PINIA_DATA__ = ${serialize(pinia.state.value)};${commonInject}`,
            })
        ...
        const customeFooterScriptArr: VNode[] = getUserScriptVue(
          customeFooterScript,
          ctx,
          h,
          'vue3'
  ...
  const fn = async () => {
    ...
    if (!isCsr) {
      ...
      const lF = layoutFetch
        ? layoutFetch({ store, router: value, ctx, pinia }, ctx)
        : Promise.resolve({})
      const CF = currentFetch
        ? currentFetch({ store, router: value, ctx, pinia }, ctx)
        : Promise.resolve({})
