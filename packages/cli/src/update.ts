...
export const update = async () => {
  ...
  await (
    await Promise.all(
      needCheck.map(async (p) => await axios.get(`https://registry.npmjs.org/${p}`))
    )
  ).forEach((item) => {
    const currentVersion = require(resolve(
      cwd,
      `./node_modules/${item.data.name}/package.json`
    )).version
  ...
  if (Object.keys(res).length === 0) {
    ...
    Object.entries(res).forEach(([key, value]) => {
      logGreen(
        `${key} current version is ${value.currentVersion} latest version is ${value.lastestVersion}`
      )
    })
  }
}
