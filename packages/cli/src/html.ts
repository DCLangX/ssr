...
export const generateHtml = async () => {
  ...
  combine[0].arr = combine[0].arr.concat([
    {
      content: `window.__USE_SSR__=false;window.__USE_VITE__=${isVite}; window.prefix="${prefix}" ;${
  ...
  if (framework === 'ssr-plugin-vue3') {
    ...
    for (const item of combine) {
      ...
      if (flag === 'header') {
        jsHeaderManifest = (await renderToString(h('div', {}, scriptArr)))
          .replace('<div>', '')
          .replace('</div>', '')
      } else {
        jsFooterManifest = (await renderToString(h('div', {}, scriptArr)))
          .replace('<div>', '')
          .replace('</div>', '')
      }
    }
  ...
  if (framework === 'ssr-plugin-vue') {
    for (const item of combine) {
      const { arr, flag } = item
      const scriptArr = arr.map(
        (item) =>
          `<script ${isVite ? 'type="module"' : ''} ${
            item.describe && 'attrs' in item.describe
              ? `src="${item.describe.attrs.src}" type=text/javascript`
              : ''
          }>${item.content} </script>`
      )
      if (flag === 'header') {
  ...
  jsManifest = jsManifest.map(
    (item) => `<script src="${manifest[item]}" ${isVite ? 'type="module"' : ''}></script>`
  )
  ...
  if (typeof cssOrderPriority === 'function' || typeof jsOrderPriority === 'function') {
    logWarning(
      'Notices: orderPriority cannot get chunkName in spa html build, you will get chunkName as undefined'
    )
  }
